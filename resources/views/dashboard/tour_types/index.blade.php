                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">Tour Type</h4>
                                </div>
                                            ...
                                            <table class="table zero-configuration">
                                                ...
                                                <tbody>
                                                    ...
                                                    @foreach($types as $type)
                                                        <tr>
                                                            <td>{{ $i }}</td>
                                                            <td>{{ $type->translate(app()->getLocale(), true)->name ?? '—' }}</td>
                                                            <td>
                                                            ...
                                                            <td class="text-right">
                                                                <a class="btn btn-primary btn-icon btn-circle btn-sm"
                                                                   href="{{ route('tour_type.edit', $type->id) }}" title="Edit">
                                                                    <i class="fas fa-edit"></i>
                                                                </a>
                                                                <a class="btn btn-primary btn-icon btn-circle btn-sm"
                                                                   href="{{ route('tour_type.delete', $type->id) }}"
        ...
        $(document).ready(function () {
            $(".zero-configuration").dataTable({
                ...
                "fnStateSave": function (oSettings, oData) {
                    localStorage.setItem('dataTable_tour_types', JSON.stringify(oData));
                },
                "fnStateLoad": function (oSettings) {
                    return JSON.parse(localStorage.getItem('dataTable_tour_types'));
                },
            });
        });
    </script>

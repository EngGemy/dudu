...
<html lang="{{ app()->getLocale() }}" dir="{{ in_array(app()->getLocale(), ['ar','he']) ? 'rtl' : 'ltr' }}">
  <head>
    ...
    <title>{{ __('front.site.footer.careers') }}</title>

    @include('front.layouts.hreflang')
      <meta itemprop="name" content="{{ $career_body?->meta_title }}">
      <meta itemprop="description" content="{{ $career_body?->meta_description }}">
      <meta itemprop="image" content="{{ $career_body?->meta_img }}">
  ...
  <body>
    <div class="app">
      <header class="page-header">
        ...
        <div class="hero">
          <div class="container">
            <div class="hero_content">
              <h1 class="txt-shadow">{{$career_header->title ?? "Join Our Team!"}}</h1>
              <p class="txt-shadow">
                  @isset($career_header->description)
                {!! implode(' ', array_slice(str_word_count($career_header->description, 1), 0, 20)) !!}
                  @endisset
        ...
        <img
          src="{{$career_header->image_url ?? './assets/images/careers-bg.jpeg'}}"
          ...
          <div class="pt-8">
            <div class="container">
              <ol class="breadcrumb mb-8" aria-label="Breadcrumb">
                ...
                <li aria-current="page">{{ __('front.site.footer.careers') }}</li>
              </ol>
              ...
              <div class="mb-12 flex flex-col gap-7 lg:flex-row">
                <div class="w-full flex-1 max-lg:order-2">
                  ...
                  >
                    <span class="text-secondary"></span>{{$career_body->title ?? "Default Title"}}
                  </h2>

                  <p>
                  {!!$career_body->description ?? "Default Description"!!}
                  </p>
                </div>

                <div class="w-full max-w-[510px] max-lg:order-1">
                  <img
                    src="{{$career_body->image_url ?? './assets/images/careers.jpeg'}}"
              ...
              <div class="mb-6 grid gap-4 lg:grid-cols-3">
                ...
                >
                  ...
                  <div>
                    ...
                    <p class="text-gray">{{ $settings->location ?? 'location' }}</p>
                  </div>
                </div>
                <div
                  class="flex items-center gap-3 rounded-xl border border-primary p-8"
                >
                  ...
                  <div>
                    ...
                    <p class="text-gray">{{ $settings->email ?? 'email' }}</p>
                  </div>
                </div>
                <div
                  class="flex items-center gap-3 rounded-xl border border-primary p-8"
                >
                  ...
                  <div>
                    ...
                    <p class="text-gray">{{$settings->phone ?? '(+20) 0123 456 4567'}}</p>
                  </div>
                </div>
              </div>

              <div class="rounded-xl border border-primary p-8 lg:p-12">
                <form id="JoinOurTeam" method="POST" action="#" enctype="multipart/form-data">
                  @csrf
                  <div
                  ...
                  >
                    ...
                    <div class="w-full space-y-6 lg:w-1/2">
                      ...
                      <div class="relative flex-1">
                        <label
                          for="message"
                          class="absolute end-4 top-0 -translate-y-1/2 bg-white px-1 text-sm text-primary lg:text-base"
                          >Upload Resume</label
                        >
                        <input
                          type="file"
                          name="resume"
                          class="w-full cursor-pointer rounded-xl border border-primary bg-transparent text-gray file:mr-4 file:cursor-pointer file:border-0 file:bg-primary file:px-4 file:py-3 file:font-poppins file:text-white file:hover:bg-secondary"
                        />
                      </div>
        ...
        <div class="flex w-full flex-col overflow-hidden rounded-3xl bg-white">
          ...
          <div class="px-6 py-8 text-center">
            ...
            <p class="mb-7 lg:mb-10 lg:text-lg full-message">
              Your resume not send. Please complete the form.
...
<script>
  $(document).ready(function() {
      ...
      $('#JoinOurTeam').submit(function(e) {
          e.preventDefault(); // Prevent default form submission
          var form = $('#JoinOurTeam')[0];
          var formData = new FormData(form);

          $("#btnSubmit").prop('disabled', true);

          $.ajax({
              url: "{{ route('join_our_teams') }}", // Form action URL
